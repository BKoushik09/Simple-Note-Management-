from flask import Flask, jsonify, request, redirect, url_for, render_template, flash, session, send_file
...
def register():
    ...
    try:
        ...
        if email_count == 0:
            gotp = getotp()
            userdata = {'username': username, 'useremail': useremail, 'userpassword': userpassword, 'userphone': userphone, 'serverotp': gotp}
...
def viewallnotes():
    ...
    try:
        ...
        cursor.execute('select userid from userdata where useremail = %s', [session.get('user')])
        user_id = cursor.fetchone()[0]
        cursor.execute('select notes_id, title, created_at from notesdata where userid = %s', [user_id])
...
def viewnotes(nid):
    ...
    try:
        ...
        cursor.execute('select userid from userdata where useremail = %s', [session.get('user')])
        user_id = cursor.fetchone()[0]
        cursor.execute('select notes_id, title, description, created_at from notesdata where userid = %s and notes_id = %s', [user_id, nid])
...
def updatenotes(nid):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.execute('select userid from userdata where useremail = %s', [session.get('user')])
            user_id = cursor.fetchone()[0]
            cursor.execute('update notesdata set title = %s, description = %s where userid = %s and notes_id = %s', [updated_title, updated_description, user_id, nid])
            mydb.commit()
            cursor.close()
        except Exception as e:
            print(e)
...
def getexceldata():
    ...
    try:
        ...
        cursor.execute('select userid from userdata where useremail=%s',[session.get('user')])
        user_id=cursor.fetchone()[0]
        cursor.execute('select notes_id,title,description,created_at from notesdata where userid=%s',[user_id])
...
def deletefiles(fid):
    ...
    try:
        ...
        print(e)
        flash("Could not delete files details")
        return redirect(url_for('dashboard'))
    return redirect(url_for('viewallfiles'))
